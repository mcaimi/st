...
static unsigned long *convert_pixmap(void);
...
evcol(XEvent *e)
{
  int x = e->xbutton.x - win.dynborder_w;
  LIMIT(x, 0, win.tw - 1);
  return x / win.cw;
}
...
evrow(XEvent *e)
{
  int y = e->xbutton.y - win.dynborder_h;
  LIMIT(y, 0, win.th - 1);
  return y / win.ch;
}
...
xtermclear(int col1, int row1, int col2, int row2)
{
  XftDrawRect(xw.draw,
      ...
      win.dynborder_w + col1 * win.cw,
      win.dynborder_h + row1 * win.ch,
      (col2-col1+1) * win.cw,
      (row2-row1+1) * win.ch);
}
...
xhints(void)
{
  ...
  sizeh->base_height = 2 * win.dynborder_w;
  sizeh->base_width = 2 * win.dynborder_h;
  sizeh->min_height = win.ch + 2 * win.dynborder_h;
  sizeh->min_width = win.cw + 2 * win.dynborder_w;
  if (xw.isfixed) {
...
xinit(int w, int h)
{
  ...
  xw.netwmiconname = XInternAtom(xw.dpy, "_NET__WM_ICON_NAME", False);
...
{
  float winx = win.dynborder_w + x * win.cw, winy = win.dynborder_h + y * win.ch, xp, yp;
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y)
{
  ...
  int winx = win.dynborder_w + x * win.cw, winy = win.dynborder_h + y * win.ch,
  ...
  if (x == 0) {
    xclear(0, (y == 0)? 0 : winy, win.dynborder_h,
      winy + win.ch +
      ((winy + win.ch >= win.dynborder_h + win.th)? win.h : 0));
  }
  if (winx + width >= win.dynborder_w + win.tw) {
    xclear(winx + width, (y == 0)? 0 : winy, win.w,
      ((winy + win.ch >= win.dynborder_w + win.th)? win.h : (winy + win.ch)));
  }
  if (y == 0)
    xclear(winx, 0, winx + width, win.dynborder_w);
  if (winy + win.ch >= win.dynborder_h + win.th)
  ...
  if (IS_SET(MODE_FOCUSED)) {
    ...
    case 4: /* Steady Underline */
      XftDrawRect(xw.draw, &drawcol,
          win.dynborder_w + cx * win.cw,
          win.dynborder_h + (cy + 1) * win.ch - \
            cursorthickness,
    ...
    XftDrawRect(xw.draw, &drawcol,
        ...
        1, win.ch - 1);
    XftDrawRect(xw.draw, &drawcol,
        win.dynborder_w + (cx + 1) * win.cw - 1,
        win.dynborder_h + cy * win.ch,
        1, win.ch - 1);
    XftDrawRect(xw.draw, &drawcol,
        win.dynborder_w + cx * win.cw,
        win.dynborder_h + (cy + 1) * win.ch - 1,
        win.cw, 1);
  }
}
...
xfinishdraw(void)
{
  ...
  for (im = term.images; im; im = im->next) {
    if (!ISNULL(im)) {
      ...
      memset(&gcvalues, 0, sizeof(gcvalues));
      gcvalues.graphics_exposures = False;
      gc = XCreateGC(xw.dpy, (USE_ARGB)?xw.buf:xw.win, GCGraphicsExposures, &gcvalues);
...
xximspot(int x, int y)
{
  ...
  xw.ime.spot.x = win.dynborder_w + x * win.cw;
  xw.ime.spot.y = win.dynborder_h + (y + 1) * win.ch;
